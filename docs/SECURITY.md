...
- Compensation must not hide the initiating failure. A release failure emits one redacted `billing-compensation-failed` event and stable response while retaining the reservation for recovery. Record successful provider outcomes before capture and mark them complete afterward; capture retry or process recreation must not cause a second provider call or charge. The committed outcome store is test-only, and production requires durable workflow/outbox state and reconciliation.
...
- Reject storage objects outside allowed size and region policy. Generate provider keys internally. Require SHA-256 or stronger integrity metadata, append immutable versions and scan results, and derive current scan state without rewriting historical versions. Parse UTC expiry/retention instants, require future restricted/regulated retention, and permit shortening only through a reasoned, audited platform-admin override. Deny download for deletion, missing governance policy, out-of-range TTL, checksum failure, incomplete scanning, or malware.
...
- Keep notification templates immutable by version. Check product/region/channel binding, channel preference and opt-out before enqueue, validate webhook destination references through a tenant allowlist, and reject conflicting deduplication reuse. Claim and commit a lease before calling a provider outside the UnitOfWork; pass a stable delivery key; finalize afterward; recover expired claims; enforce retry due times; and ensure cancellation, suppression, delivery, and dead-letter states terminally resolve the outbox. Redact provider responses and rely on provider-key replay after acceptance/finalization failure.
- Commit a business mutation and its outbox record atomically wherever the service owns the transaction, including tenant-status changes. Event IDs fingerprint event type, tenant, request, and safe attributes. Validate UTC/schema/stable IDs and deeply immutable allowlisted scalar attributes. Use locked immutable records, optimistic transition versions, exclusive expiring leases, enforced due times and attempt limits, acceptance-before-acknowledgement, retry-safe downstream deduplication, terminal cancellation, and visible poison-event dead letters. Publication failure after commit must never change the business response.
- Analytics accepts fixed operational dimensions only. It excludes prompts, model outputs, message bodies, file content, credentials, payment data, direct personal data, and arbitrary user metadata. The service owns recording time, enforces late/future-skew bounds, uses parsed half-open UTC windows, and applies minimum-count suppression to every tenant and cross-tenant export. Cross-tenant results contain aggregate points only.
- Durable audit accepts explicit allowlisted scalar attributes only and always marks events redacted. Exclude authorization headers, tokens, secrets, bodies, prompts, outputs, notification content, payment data, stack traces, and arbitrary exception text. Hash canonical tenant-scoped events into monotonic chains; verify the complete contiguous chain from genesis through every checkpoint; verify full source integrity and exact boundaries before range export; and hash canonical exported content. Source event IDs are idempotent, conflicting reuse fails closed, and direct callers cannot assign another authenticated actor.
- HTTP boundaries cap bodies at 65,536 bytes, constrain request/correlation identifiers, return `no-store`, paginate reads, and expose stable redacted status contracts. The HTTP layer owns exactly one audit event for each rejected request and exactly one for each unexpected internal failure; provider/infrastructure failures are never mislabeled as policy denial. Errors never expose provider responses, bodies, storage keys, opaque tokens, stack traces, or repository details.

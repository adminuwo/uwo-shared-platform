...
class PlatformAnalyticsService:
    def __init__(self,uow:UnitOfWorkFactory,authorizer:DataServiceAuthorizer,audit:AuditSink,*,minimum_export_count:int=5,max_lateness_seconds:int=86400,clock:Callable[[],str]=utc_now)->None:
        self._uow=uow; self._auth=authorizer; self._audit=audit; self._minimum=minimum_export_count; self._lateness=max_lateness_seconds; self._clock=clock
    def ingest(self,identity,tenant_id,event:AnalyticsEvent,idempotency_key,request_id):
        ...
        cutoff=datetime.fromisoformat(self._clock())-timedelta(seconds=self._lateness)
        if datetime.fromisoformat(event.occurred_at)<cutoff: raise PolicyViolation("late_event_rejected","event exceeds the configured lateness window")
        fp=contract_fingerprint(event); scope=("analytics.ingest",tenant_id)
        with self._uow() as tx:
            ...
            result=tx.events.append(event); tx.idempotency.put(scope,idempotency_key,fp,result)
            tx.outbox.enqueue(OutboxRecord(deterministic_id("outbox",event.event_id),platform_event("analytics.event.accepted",tenant_id,request_id,{"resource_id":event.event_id,"region":event.region,"product":event.product.value},event.recorded_at),OutboxStatus.PENDING,0,None,1)); tx.commit()
        return result
    ...
    def aggregate(self,identity,tenant_id,window:AggregationWindow):
        ...
        return tuple(MetricPoint(deterministic_id("metric",et.value),tenant_id,p,r,et,window,count) for (p,r,et),count in sorted(groups.items(),key=lambda x:(x[0][0].value,x[0][1],x[0][2].value)))
    ...
    def cross_tenant_export(self,identity,snapshot_ids):
        self._auth.require_platform_admin(identity); values=[]
        with self._uow() as tx:

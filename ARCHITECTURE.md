...
Secure execution adds an authenticated edge assertion, verified tenant binding, product/model entitlements, and billing authorization before provider selection. Provider adapters resolve credentials through a secret-manager contract only at execution time. Azure OpenAI and OpenAI scaffolds are wrapped by bounded timeouts, retry, fallback, and per-provider circuit breakers. Structured audit events contain allowlisted identifiers and decisions, never prompts, outputs, bearer tokens, or credential values.
